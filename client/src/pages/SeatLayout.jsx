...
const SeatLayout = () => {
    ...
    <div key={row} className='flex gap-2 mt-2'>
      <div className='flex flex-wrap items-center justify-center gap-2'>
        {Array.from({ length: count }, (_, i) => {
          ...
          return (
            <button key={seatId} onClick={() => handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId) && 'bg-primary text-white'}  ${occupiedSeats.includes(seatId) && 'bg-gray-600 text-white cursor-not-allowed'}`
            }>
              {seatId}
            </button>
          );
  ...
  const getOccupiedSeats = async() => {
    try {
      ...
      // console.log("OCCUPIED FROM API:", data);
      if(data.success){
